...
import { getAllPMRequests } from "@/lib/queries/pm";
...
export default async function PMRequestsPage({
  ...
  const requests = await getAllPMRequests(DEMO_PM_ID, statusFilter);
  ...
  return (
    <DashboardShell user={{ name: fullName, role: "property_manager" }} sidebarLinks={SIDEBAR}>
      ...
      <StatusFilterBar basePath="/requests/pm" current={statusFilter} />
