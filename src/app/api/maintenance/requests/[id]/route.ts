...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!requestId || !userId || !role) {
    return NextResponse.json({ error: 'A valid request id, userId, and role are required.' }, { status: 400 });
  }

  try {
    const maintenanceRequest = await getMaintenanceRequestDetail(requestId, userId, role);
    ...
    return NextResponse.json({ error: 'Failed to load maintenance request detail.' }, { status: 500 });
  }
}
